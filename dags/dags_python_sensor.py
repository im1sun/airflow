...
with DAG(
    ...
    def check_api_update(http_conn_id, endpoint, base_dt_col, **kwargs):
        ...
        print("111111111",last_dt)
        # last_date = last_dt[:10]
        # last_date = last_date.replace('.', '-').replace('/', '-')
        last_date = datetime.strptime(last_dt, "%Y%m%d").strftime("%Y-%m-%d")
        print("2222222222", last_date)
        try:
    ...
    sensor_task = PythonSensor(
        ...
        op_kwargs={'http_conn_id':'openapi.seoul.go.kr',
                   'endpoint':'{{var.value.apikey_openapi_seoul_go_kr}}/json/SeoulLibNewArrivalInfo',
                   'base_dt_col':'INDT'},
